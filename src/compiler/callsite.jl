...
function headstring(@nospecialize(T))
    ...
    elseif T isa UnionAll
        return headstring(unwrap_unionall(T))
    else
...
function show_callinfo(limiter, ci::EdgeCallInfo)
    mi = ci.edge.def
    tt = (unwrap_unionall(mi.specTypes)::DataType).parameters[2:end]
...
function maybe_callsite(call::CodeInstance, callee::MethodInstance)
    ...
    if Base.isvatuple(Tcall) && Base.isvatuple(Tcallee)
        Tcall, Tcallee = unwrap_unionall(Tcall), unwrap_unionall(Tcallee)
...
function show_callsite(io::IO, c::Callsite, info::EdgeCallInfo)
    optimize = get(io, :optimize, true)::Bool
    optimize && print(io, c.head, ' ')
    show_callinfo(io, info)
end
